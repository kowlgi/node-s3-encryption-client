...
const metadataCipherAlgorithm = 'cipher-algorithm',
      ...
      metadataKmsKeyName = 'key';

var   objects = {},
      objectTimestamps = {},
      cacheForMillis = 0;

exports.getObject = function(config, params, callback) {
        ...
        if (kmsKeyBase64) {
          ...
          kms.decrypt({CiphertextBlob: kmsKeyBuffer, EncryptionContext: encryptionContext}, function(err, kmsData) {
            if (err) {
              ...
              delete objectData.Metadata['key'];
...
exports.putObject = function(config, params, callback) {
  AWS.config.update(config);  
  var kms = new AWS.KMS(),
  var s3 = new AWS.S3();
